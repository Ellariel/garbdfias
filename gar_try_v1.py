...
hadobj['OKTMO'] = hadobj['OKTMO'].apply(lambda x: x[0] if isinstance(x, list) else None)
hadobj['KLADR'] = hadobj['KLADR'].apply(lambda x: x[0] if isinstance(x, list) else None)
...
def get_town(x):
    ...
    Chain post-cleanup.
    """
    town = [p for p in ['5', '6', '4', '1'] if x[p] == 1]
    town = town[0] if len(town) > 0 else None

    street = [p for p in ['7', '8'] if x[p] == 1]
    street = street[0] if len(street) > 0 else None
    
    mun = [p for p in ['3', '2'] if x[p] == 1]
    mun = mun[0] if len(mun) > 0 else None
    
    leftover = [
        p for p in [f'{i}' for i in range(8, 3, -1)]
        if p != town
        and p != street
        and p != mun
        and x[p] == 1
...
df['leftover'] = [x[3] for x in lst]

df['levchain'] = df['levchain'].apply(lambda x: tuple(x.split('-')))

#df.head()
dfch = dfch.merge(df[['levchain', 'street', 'tow', 'leftover', 'mun']], on='levchain')
dfch['id_reg'] = region
